...
function job_setup()
    ...
    blue_magic_maps.Magical = S{
		...
		'Palling Salvo','Tenebral Crush','Blinding Fulgor'
    }
    ...
    -- Magical spells with a Agi stat mod (on top of Int)
    ...
    unbridled_spells = S{
		...
		'Polar Roar','Pyric Bulwark','Thunderbolt','Tourbillion','Uproot'
    }
end
...
function init_gear_sets()
	...
	Rosmerta = {}
	Rosmerta.STR    = {name="Rosmerta's Cape", augments={'STR+20','Accuracy+20 Attack+20','Weapon skill damage +1%'}}
...
function job_handle_equipping_gear(playerStatus, eventArgs)    	
    ...
    if player.equipment.neck == "Adoulin's Refuge +1" 
	then
        disable('neck')
    else
        enable('neck')
    ...
    if player.equipment.sub == 'Homestead Blade' 
	then
        disable('sub')
    else
        enable('sub')
...
function customize_melee_set(meleeSet)
    ...
    if buffactive["Aftermath: Lv.3"] and player.equipment.main == 'Tizona' then
        meleeSet = set_combine(meleeSet, {back=Rosmerta.Tizona})
    end
    return meleeSet
end
